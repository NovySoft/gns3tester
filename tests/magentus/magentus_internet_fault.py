...
async def main():
    ...
    final_output = open('./tests/magentus/magentus_internet_fault.md', 'w', encoding='utf-8')
    final_output.write("# Magentus \"Internet Fault\" tesztelési jegyzőkönyv\n\n")
    ...
    svg_data = f.read()
    f.close()
    internet_devices = list(filter(lambda d: ("edge-r3" in d['name'].lower()) or ("edge-r4" in d['name'].lower()) or ("ip6" in d['name'].lower()), magentus_devices))

    # Make dsabled path red, and save to a new SVG file, and write to documentation
    soup = BeautifulSoup(svg_data, 'xml')
    for router in internet_devices:
    ...
    if not os.path.exists(f'./tests/magentus/images/fault1_router/internet/'):
        os.makedirs(f'./tests/magentus/images/fault1_router/internet/')        
    f = open(f'./tests/magentus/images/fault1_router/internet/internet.svg', 'w', encoding='utf-8')
    f.write(str(soup))
    f.close()
    print("Success: SVG Path found and made red, written to docs.")
    print(f"Disabling router  waiting 10 seconds for ospf to converge...")
    for router in internet_devices:
        suspend_router(router['id'], True)
    final_output.write(f"## Hiba szimuláció: {router['name']}\n\n")
    final_output.write(f"<img src=\"images/fault1_router/internet/internet.svg\" style=\"display: block; margin: 0 auto; width: 500px;\">\n\n")
    ...
    for i in range(len(results)):
        red_line_svg_file = open(f'./tests/magentus/images/fault1_router/internet/internet.svg', 'r', encoding='utf-8')
                ...
                if ip_in_line[0] == '10.58.2.1':
                    ...
                    while not prev_hop.strip().split(' ')[0].isdigit():
                        ...
                        if len(prev_ip_in_line) > 0:
                            link_id = final_hop_links.get(prev_ip_in_line[0])
                            make_path_dotted_orange(soup, link_id)
            f = open(f'./tests/magentus/images/fault1_router/internet/internet-{customer}.svg', 'w', encoding='utf-8')
            f.write(str(soup))
            f.close()
            final_output.write(f'    <img src="./images/fault1_router/internet/internet-{customer}.svg" width="100%">\n')
        else:
