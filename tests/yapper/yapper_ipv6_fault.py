...
async def main():
    ...
    final_output = open('./tests/yapper/yapper_ipv6_fault.md', 'w', encoding='utf-8')
    final_output.write("# Yapper \"IPv6\" tesztelési jegyzőkönyv\n\n")
    ...
    for router in yapper_devices:
        ...
        if not os.path.exists(f'./tests/yapper/images/ipv6/{router['id']}/'):
            os.makedirs(f'./tests/yapper/images/ipv6/{router['id']}/')  
        f = open(f'./tests/yapper/images/ipv6/{router['id']}/{router['id']}.svg', 'w', encoding='utf-8')
        f.write(str(soup))
        f.close()
        ...
        for i in range(len(results)):
            red_line_svg_file = open(f'./tests/yapper/images/ipv6/{router["id"]}/{router["id"]}.svg', 'r', encoding='utf-8')
            ...
            if (len(result) > 0 and '2001:4860:4860::8888' in result[0]) or \
                ...
                for j in range(len(result)):
                    ...
                    if '2001:470:2171:aaaa:aaaa:0001::' in ip_in_line[0].lower() or '2001:470:2171:aaaa:aaaa:1:' in ip_in_line[0].lower(): # MESH 1 network, is a switch need to find previous link
                        temp_j = j
                        prev_hop = result[temp_j-1]
                        prev_ip_in_line = re.findall(r'[a-fA-F0-9]{1,4}(?::[a-fA-F0-9]{0,4})+', prev_hop)
                        ...
                        while not prev_hop.strip().split(' ')[0].isdigit():
                            ...
                            if len(prev_ip_in_line) > 0:
                                ...
                                for r in all_yapper_devices:
                                    for p in r['ports']:
                                        if p != None and p.get('ipv6', '') != None and p.get('ipv6', '').lower().split('/')[0] == prev_ip_in_line[0].lower():
                                            routerp = r
                                            break
                                if routerp is None:
                                ...
                                for sport in routerp['ports']:
                                    if sport['connected_to'] != 'Unconnected' and sport['connected_to'] != None:
                                        if sport['connected_to']['name'] == 'YAPPER-MESH1':
                                            ...
                                            break
                    elif '2001:470:2171:aaaa:aaaa:0002::' in ip_in_line[0].lower() or '2001:470:2171:aaaa:aaaa:2:' in ip_in_line[0].lower(): # MESH 2 network, is a switch need to find previous link
                        temp_j = j
                        prev_hop = result[temp_j-1]
                        prev_ip_in_line = re.findall(r'[a-fA-F0-9]{1,4}(?::[a-fA-F0-9]{0,4})+', prev_hop)
                        need_prev_hop = (not result[temp_j].strip().split(' ')[0].isdigit())
                                ...
                                for sport in routerp['ports']:
                                    if sport['connected_to'] != 'Unconnected' and sport['connected_to'] != None:
                                        if sport['connected_to']['name'] == 'YAPPER-MESH2':
                                            ...
                                            break
                f = open(f'./tests/yapper/images/ipv6/{router["id"]}/{router["id"]}-{customer}.svg', 'w', encoding='utf-8')
                f.write(str(soup))
                f.close()
