...
async def run_traceroute_v6_telnet(reader, writer, target_ip, source_ip, result_lines, device="Unknown"):
    """
    ...
    writer.write('traceroute\r')
    await asyncio.sleep(1)
    writer.write('ipv6\r') # protocol
    await asyncio.sleep(1)
    ...
    await asyncio.sleep(1)
    writer.write('\r') # Minium TTL
    await asyncio.sleep(1)
    writer.write('\r') # Maximum TTL
    await asyncio.sleep(1)
    writer.write('\r') # Pirority
    await asyncio.sleep(1)
    writer.write('\r') # Port number

    writer.write(f"{target_ip}\r\n")

    print(f"Device {device} executing: traceroute {target_ip} numeric source {source_ip}", flush=True)

    # Traceroute takes time. 
    # Adjusted sleep to 10s. Increase this if your network hops are very high latency.
